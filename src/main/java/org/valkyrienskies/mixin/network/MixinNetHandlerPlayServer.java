...
public abstract class MixinNetHandlerPlayServer {
    ...
    private List<AxisAlignedBB> removeStuckInBlockMovementCheck(WorldServer worldServer, Entity entityIn, AxisAlignedBB aabb) {
        ...
        if (draggable == null) throw new RuntimeException("IEntityShipDraggable is not expected to be null!");

        if (draggable.getLastTouchedShip() != null) return ImmutableList.of();
        else return player.getServerWorld().getCollisionBoxes(player, player.getEntityBoundingBox().shrink(0.0625F));
    }
    ...
    private double allowMovementToBeVeryWrong(double originalConstant) {
        ...
        if (draggable.getLastTouchedShip() != null) return 10;
        else return 0.0625;
    }
        ...
        if (this.player.getServerWorld().isCallingFromMinecraftThread()) {
            ...
            if (ticksSinceTouchedLastShip > 40) {
                ...
                if (entityShipDraggable != null) {
                    ...
                    entityShipDraggable.setTicksPartOfGround(addedPlayerMovementData.getTicksPartOfGround());
                    entityShipDraggable.setTicksSinceTouchedShip(ticksSinceTouchedLastShip);
                }
                return;
            }
