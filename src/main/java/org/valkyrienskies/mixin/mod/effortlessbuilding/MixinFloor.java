...
@Pseudo
@Mixin(targets = "nl.requios.effortlessbuilding.buildmode.buildmodes.Floor", remap = false)
public abstract class MixinFloor {
    @ModifyVariable(method = "findFloor", at = @At("STORE"), index = 3, remap = false)
    private static Vec3d useShipLocalLook(
            Vec3d look, @Local(argsOnly = true) EntityPlayer player, @Local(argsOnly = true) BlockPos firstPosition
    ) {
    ...
    @ModifyVariable(method = "findFloor", at = @At("STORE"), index = 4, remap = false)
